...
class CoaDetail extends EntityBase {
    ...
    public function LoadAkunNeraca($entityId = 0, $cabangId = 0, $orderBy = "a.kode") {
        $sql = "SELECT a.*, b.psaldo FROM m_account AS a JOIN m_lk_rekap_detail AS b ON a.kd_induk = b.kd_induk JOIN m_lk_rekap_master AS c ON b.kd_kelompok = c.kd_kelompok WHERE a.is_deleted = 0 and c.nm_laporan = 'NERACA'";
        if ($entityId > 0){
            $sql.= " And a.entity_id = ".$entityId;
        }
        if ($cabangId > 0){
    ...
    public function LoadAkunLabaRugi($entityId = 0, $cabangId = 0, $orderBy = "a.kode") {
        $sql = "SELECT a.*, b.psaldo FROM m_account AS a JOIN m_lk_rekap_detail AS b ON a.kd_induk = b.kd_induk JOIN m_lk_rekap_master AS c ON b.kd_kelompok = c.kd_kelompok WHERE a.is_deleted = 0 and c.nm_laporan = 'LABA RUGI'";
        if ($entityId > 0){
            $sql.= " And a.entity_id = ".$entityId;
        }
        if ($cabangId > 0){
