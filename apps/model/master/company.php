...
'INSERT INTO sys_company(bpjs_kes,fee_dokter_jaga,saldo_awal,entity_cd,company_name,address,city,province,telephone,facsimile,npwp,personincharge,pic_status,start_date)
VALUES(?bpjs_kes,?fee_dokter_jaga,?saldo_awal,?entity_cd,?company_name,?address,?city,?province,?telephone,?facsimile,?npwp,?personincharge,?pic_status,?start_date)';
        ...
        $this->connector->AddParameter("?saldo_awal", $this->SaldoAwal);
        $this->connector->AddParameter("?bpjs_kes", $this->BpjsKes);
        $this->connector->AddParameter("?fee_dokter_jaga", $this->FeeDokterJaga);
...
'UPDATE sys_company SET
	...
	bpjs_kes = ?bpjs_kes,
	fee_dokter_jaga = ?fee_dokter_jaga
...
//		$this->connector->AddParameter("?id", $id);
		$this->connector->CommandText = "UPDATE sys_company SET is_deleted = 1 WHERE entity_id = ?id";
		$this->connector->AddParameter("?id", $id);
		return $this->connector->ExecuteNonQuery();
	}
