...
ORDER BY a.resource_id";
    ...
    public function LoadUserCabAcl($uid){
        $sql = "Select group_concat(a.cabang_id) as valresult from vw_sys_user_cab_acl a where a.user_uid = ".$uid;
        $this->connector->CommandText = $sql;
        $rs = $this->connector->ExecuteQuery();
        $row = $rs->FetchAssoc();
    ...
    public function GetUserAclList($uid){
        $sql = "Select * From vw_sys_user_acl_lists a Where a.user_uid = $uid Order By a.user_id, a.sbu_name, a.menu_name, a.resource_name";
        $this->connector->CommandText = $sql;
        $rs = $this->connector->ExecuteQuery();
        return $rs;
    }
