...
class TrxType extends EntityBase {
	...
	public function Insert() {
		$this->connector->CommandText = "INSERT INTO sys_trxtype(trx_code, entity_id, cabang_id, trx_mode, trx_descs, trx_acc_no, refftype_id, createby_id, create_time, def_acc_no) VALUES(?trx_code, ?entity_id, ?cabang_id, ?trx_mode, ?trx_descs, ?trx_acc_no, ?refftype_id, ?createby_id, NOW(), ?def_acc_no)";
...
WHERE id = ?id";
    ...
    public function GetAutoKode($entityId = 0, $trxMode = 0) {
        // function untuk menggenerate kode sku
        if ($trxMode == 1) {
            $kode = "TD" . $entityId . "001";
        }elseif ($trxMode == 2){
            $kode = "TK" . $entityId . "001";
        }else{
            $kode = "TN" . $entityId . "001";
        }
        $sqx = "SELECT max(a.trx_code) AS pKode FROM sys_trxtype a WHERE a.entity_id = $entityId And left(a.trx_code,3) = '".left($kode,3)."'";
        ...
        if ($rs != null) {
            ...
            if (strlen($pkode) == 6){
                ...
                $kode = left($kode,3).str_pad($counter,3,'0',STR_PAD_LEFT);
            }
        }
        return $kode;
    }
