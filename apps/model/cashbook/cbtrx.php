...
class CbTrx extends EntityBase {
    ...
    public function LoadByTrxTypeCode($trxTypeId) {
        $this->connector->CommandText = "SELECT a.* FROM t_cb_transaction AS a WHERE a.is_deleted = 0 and a.trxtype_code = ?trxTypeId";
    ...
    public function Load4Reports($entityId,$cabangId = 0, $trxTypeId = 0, $trxMode = 0, $bankId = 0, $trxStatus = -1, $startDate = null, $endDate = null) {
        ...
        if ($trxTypeId > 0){
            $sql.= " and a.trxtype_code = ".$trxTypeId;
        }
        if ($bankId > 0){
            $sql.= " and (a.bank_id = '".$bankId."' or a.bank_id = '".$bankId."')";
        }
        if ($cabangId > 0){
    ...
    public function LoadRekoran($cabangId = 0, $bankId = 0, $startDate = null, $endDate = null) {
        ...
        $sql.= " From vw_t_cb_transaction a";
	...
	public function Insert() {
		$this->connector->CommandText = "INSERT INTO t_cb_transaction(sbu_id,dept_cd,entity_id,cabang_id,trx_date,trx_mode,trxtype_code,bank_id,doc_no,trx_descs,db_acc_no,cr_acc_no,trx_amount,contact_id,reff_no,trx_status,createby_id,create_time,create_mode)
		VALUES(?sbu_id,?dept_cd,?entity_id,?cabang_id,?trx_date,?trx_mode,?trxtype_code,?bank_id,?doc_no,?trx_descs,?db_acc_no,?cr_acc_no,?trx_amount,?contact_id,?reff_no,?trx_status,?createby_id,NOW(),?create_mode)";
