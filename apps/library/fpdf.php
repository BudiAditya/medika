...
class FPDF
{
	...
	*******************************************************************************/
	public function FPDF($orientation='P', $unit='mm', $size='A4')
	{
		...
		// Page sizes
		$this->StdPageSizes = array('a3'=>array(841.89,1190.55), 'a4'=>array(595.28,841.89), 'a5'=>array(420.94,595.28),'a5l'=>array(595.28,420.94),
			'letter'=>array(612,792), 'legal'=>array(612,1008), 'f4'=>array(612,935.43), 'halfletter'=>array(612,396),);
	...
	public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
	{
		...
		if($w==0) {
			...
			// OK HACKING untuk bisa kolom dihitung dari kanan
			$w = abs($w);
			$this->x = $this->w - $this->rMargin - $w;
		}
		$s = '';
		if($fill || $border==1)
		{
	...
	public function MultiCell($width, $height, $txt, $border = 0, $align = 'L', $fill = false) {
		...
		if ($width == 0) {
			...
			$width = abs($width);
			$this->x = $this->w - $this->rMargin - $width;
		}
		...
		$j = 0;		// Counter mulai substring (Bisa sesudah \n)
		$l = 0;		// Width yang sudah di occupy oleh karakter substring
		$ns = 0;	// Jumlah spasi yang pada baris yang sedang di cari (berguna untuk justify)
		$nl = 1;	// Jumlah baris yang digunakan (By default 1)
		while ($i < $textLength) {
			// Get next character
			$c = $text[$i];
			if ($c == "\n") {
				// Explicit line break because we found ENTER char
				if ($this->ws > 0) {
					$this->ws = 0;
					$this->_out('0 Tw');
				}
				$this->Cell($width, $height, substr($text, $j, $i - $j), $b, 2, $align, $fill);
				$i++;
				$sep = -1;
				$j = $i;
				$l = 0;
			...
			if ($c == ' ') {
				// OK Store space position so we can properly substring the text
				$sep = $i;
				$ls = $l;
				$ns++;
			}
			...
			if ($l > $maxTextWidth) {
				...
				if ($sep == -1) {
					if ($i == $j) {
						// Entah kenapa tapi feeling gw sih ini ga mungkin kejadian
						// Apakah mungkin jika lebar karakternya > lebar kolom yang disediakan ?
						$i++;
					}
					if ($this->ws > 0) {
						$this->ws = 0;
					...
					if ($align == 'J') {
						$this->ws = ($ns > 1) ? ($maxTextWidth - $ls) / 1000 * $this->FontSize / ($ns - 1) : 0;
						$this->_out(sprintf('%.3F Tw', $this->ws * $this->k));
					}
					$this->Cell($width, $height, substr($text, $j, $sep - $j), $b, 2, $align, $fill);
					$i = $sep + 1;
				}
				$sep = -1;
				$j = $i;
	...
	public function SetX($x, $considerRightMargin = false)
	{
		// Set x position
		if ($x >= 0) {
			$this->x = $x;
