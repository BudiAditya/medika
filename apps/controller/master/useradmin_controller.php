...
class UserAdminController extends AppController {
	...
	public function index() {
		...
		$settings["columns"][] = array("name" => "a.user_uid", "display" => "ID", "width" => 40);
		$settings["columns"][] = array("name" => "a.user_id", "display" => "User ID", "width" => 100);
		$settings["columns"][] = array("name" => "a.user_name", "display" => "Nama Lengkap", "width" => 150);
		$settings["columns"][] = array("name" => "a.user_email", "display" => "Email", "width" => 150);
		$settings["columns"][] = array("name" => "b.sbu_name", "display" => "Unit", "width" => 50);
		...
		$settings["columns"][] = array("name" => "a.login_time", "display" => "Login Terakhir", "width" => 100);
		...
		if (!$router->IsAjaxRequest) {
			...
			if ($acl->CheckUserAccess("master.useracl", "edit")) {
				$settings["actions"][] = array("Text" => "Setting Hak Akses", "Url" => "master.useracl/add/%s", "Class" => "bt_lock", "ReqId" => 1);
			}
			$settings["actions"][] = array("Text" => "separator", "Url" => null);
			...
			$settings["from"] =
				"sys_users AS a
	LEFT JOIN m_sbu AS b On a.sbu_id = b.id
	...
	public function add() {
		...
		if (count($this->postData) > 0) {
			...
			$userAdmin->CabangId = $this->userCabangId;
			$userAdmin->UserLvl = $this->GetPostValue("UserLvl");
            $userAdmin->SbuId = $this->GetPostValue("SbuId");
            $userAdmin->EmployeeId = $this->GetPostValue("EmployeeId");
			...
			if ($this->DoInsert($userAdmin)) {
				...
				if ($this->connector->GetHasError()) {
					if ($this->connector->GetErrorCode() == $this->connector->GetDuplicateErrorCode()) {
                        $this->persistence->SaveState("error", sprintf("Data Ganda atau ID: '%s' telah ada pada database !", $userAdmin->UserId));
					} else {
                        $this->persistence->SaveState("error", sprintf("System Error: %s. Please Contact System Administrator.", $this->connector->GetErrorMessage()));
					}
				}
			}
		}
        //load data yg dibutuhkan
        $loader = new Karyawan();
        $karyawans = $loader->LoadAll();
		$loader = new UserAdmin();
		$userLvl = $loader->GetUserLevel($this->userLevel);
		$loader = new Sbu();
		$sbuList = $loader->LoadAll($this->userCompanyId);
        $this->Set("karyawans", $karyawans);
		$this->Set("userAdmin", $userAdmin);
		$this->Set("userLvl", $userLvl);
        $this->Set("sbuList", $sbuList);
	}
