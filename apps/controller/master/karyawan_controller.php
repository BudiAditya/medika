...
class KaryawanController extends AppController {
	...
	public function index() {
		...
		$settings["columns"][] = array("name" => "a.nama", "display" => "Nama Karyawan", "width" => 200);
		...
		if (!$router->IsAjaxRequest) {
			...
			$settings["from"] = "m_karyawan AS a JOIN sys_company AS b ON a.entity_id = b.entity_id LEFT JOIN sys_dept As c on a.dept_id = c.id LEFT JOIN sys_status_code AS d ON a.st_kerja = d.code And d.key = 'st_kerja'";
			if ($this->userLevel > 3) {
	...
	public function add() {
		...
		if (count($this->postData) > 0) {
			...
			$karyawan->EntityId = $this->userCompanyId;
            ...
            $karyawan->StKerja = $this->GetPostValue("StKerja");
            ...
            $karyawan->NikKtp = $this->GetPostValue("NikKtp");
            $karyawan->NoKK = $this->GetPostValue("NoKK");
			$karyawan->CreatebyId = $this->userUid;
	...
	public function edit($id = null) {
		...
		if (count($this->postData) > 0) {
            ...
            $karyawan->StKerja = $this->GetPostValue("StKerja");
            $karyawan->UpdatebyId = $this->userUid;
            $karyawan->NikKtp = $this->GetPostValue("NikKtp");
            $karyawan->NoKK = $this->GetPostValue("NoKK");
			if (!empty($_FILES['FileName']['tmp_name'])){
				$fpath = 'public/upload/images/';
