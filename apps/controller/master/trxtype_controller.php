...
class TrxTypeController extends AppController {
	...
	public function index() {
		...
		$settings["columns"][] = array("name" => "a.trx_code", "display" => "Kode", "width" => 50);
	...
	public function add() {
		...
		if (count($this->postData) > 0) {
            ...
            $trxtype->TrxCode = $trxtype->GetAutoKode($this->userCompanyId,$trxtype->TrxMode);
	...
	public function edit($id = null) {
		...
		if (count($this->postData) > 0) {
			...
			$trxtype->EntityId = $this->userCompanyId;
			$trxtype->CabangId = $this->userCabangId;
            $trxtype->TrxCode = $this->GetPostValue("TrxCode");
            $trxtype->TrxMode = $this->GetPostValue("TrxMode");
            $trxtype->TrxDescs = $this->GetPostValue("TrxDescs");
            $trxtype->TrxAccNo = $this->GetPostValue("TrxAccNo");
            $trxtype->DefAccNo = $this->GetPostValue("DefAccNo");
    ...
    public function AutoKode($trxMode = 0){
        $kode = new TrxType();
        $kode = $kode->GetAutoKode($this->userCompanyId,$trxMode);
        print $kode;
    }
}
