...
class CbTrxController extends AppController {
	...
	public function index() {
		...
		$settings["columns"][] = array("name" => "a.sbu_name", "display" => "Unit", "width" => 60);
        ...
        $settings["columns"][] = array("name" => "a.dept_cd", "display" => "Beban", "width" => 100);
		...
		if (!$router->IsAjaxRequest) {
			...
			if ($acl->CheckUserAccess("cashbook.cbtrx", "add")) {
				$settings["actions"][] = array("Text" => "Add", "Url" => "cashbook.cbtrx/add", "Class" => "bt_add", "ReqId" => 0);
			}
			if ($acl->CheckUserAccess("cashbook.cbtrx", "edit")) {
				$settings["actions"][] = array("Text" => "Edit", "Url" => "cashbook.cbtrx/edit/%s", "Class" => "bt_edit", "ReqId" => 1,
					"Error" => "Mohon memilih Data Transaksi terlebih dahulu sebelum proses edit.\nPERHATIAN: Mohon memilih tepat satu trxtype.",
					"Confirm" => "");
			}
            if ($acl->CheckUserAccess("cashbook.cbtrx", "view")) {
                $settings["actions"][] = array("Text" => "View", "Url" => "cashbook.cbtrx/view/%s", "Class" => "bt_view", "ReqId" => 1,
                    "Error" => "Maaf anda harus memilih Data Transaksi terlebih dahulu.\nPERHATIAN: Pilih tepat 1 data transaksi","Confirm" => "");
            }
			if ($acl->CheckUserAccess("cashbook.cbtrx", "delete")) {
				$settings["actions"][] = array("Text" => "Delete", "Url" => "cashbook.cbtrx/delete/%s", "Class" => "bt_delete", "ReqId" => 1,
            ...
            if ($acl->CheckUserAccess("cashbook.cbtrx", "view")) {
                $settings["actions"][] = array("Text" => "Laporan", "Url" => "cashbook.cbtrx/report", "Class" => "bt_report", "ReqId" => 0);
            }
            $settings["actions"][] = array("Text" => "separator", "Url" => null);
            if ($acl->CheckUserAccess("cashbook.cbtrx", "approve")) {
                $settings["actions"][] = array("Text" => "Approval", "Url" => "cashbook.cbtrx/approve", "Class" => "bt_approve", "ReqId" => 2,
            ...
            if ($acl->CheckUserAccess("cashbook.cbtrx", "approve")) {
                $settings["actions"][] = array("Text" => "Batal Approval", "Url" => "cashbook.cbtrx/unapprove", "Class" => "bt_reject", "ReqId" => 2,
            ...
            $settings["actions"][] = array("Text" => "separator", "Url" => null);
            if ($acl->CheckUserAccess("cashbook.cbtrx", "view")) {
                $settings["actions"][] = array("Text" => "Print Bukti", "Url" => "cashbook.cbtrx/cetakpdf/%s", "Class" => "bt_print", "ReqId" => 1,
            ...
            */
			...
			$settings["from"] = "vw_t_cb_transaction AS a";
            if ($_GET["query"] == "") {
                $_GET["query"] = null;
	...
	public function add() {
		...
		if (count($this->postData) > 0) {
            ...
            $cbtrx->SbuId = $this->GetPostValue("SbuId");
            $cbtrx->TrxDate = $this->GetPostValue("TrxDate");
            $cbtrx->TrxMode = $this->GetPostValue("TrxMode");
            $cbtrx->TrxTypeCode = $this->GetPostValue("TrxTypeCode");
            $cbtrx->TrxDescs = $this->GetPostValue("TrxDescs");
            $cbtrx->DeptCd = $this->GetPostValue("DeptCd");
			...
			if ($this->ValidateData($cbtrx)) {
                ...
                $rs = $cbtrx->Insert();
				if ($rs == 1) {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Add New CashBook Trx',$cbtrx->DocNo,'Success');
					$this->persistence->SaveState("info", sprintf("Data Data Transaksi: %s (%s) sudah berhasil disimpan", $cbtrx->DocNo, $cbtrx->TrxDescs));
					redirect_url("cashbook.cbtrx");
				} else {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Add New CashBook Trx',$cbtrx->DocNo,'Failed');
        ...
        $dept = new Department();
        $dept = $dept->LoadByEntityId($this->userCompanyId);
        $sbu = new Sbu();
        $sbu = $sbu->LoadAll($this->userCompanyId);
        $this->Set("sbus", $sbu);
	...
	public function edit($id = null) {
        ...
        if (count($this->postData) > 0) {
            ...
            if ($this->ValidateData($cbtrx)) {
                ...
                if ($rs == 1) {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Update CashBook Trx',$cbtrx->DocNo,'Success');
                    $this->persistence->SaveState("info", sprintf("Data Transaksi: %s (%s) sudah berhasil diupdate..", $cbtrx->DocNo, $cbtrx->TrxDescs));
                    redirect_url("cashbook.cbtrx");
                } else {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Update CashBook Trx',$cbtrx->DocNo,'Failed');
	...
	public function delete($id = null) {
        ...
        if($cbtrx->TrxStatus == 0){
            ...
            if ($rs == 1) {
                $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Delete CashBook Trx',$cbtrx->DocNo,'Success');
                $this->persistence->SaveState("info", sprintf("Data Transaksi: %s (%s) sudah dihapus", $cbtrx->DocNo, $cbtrx->TrxDescs));
            } else {
                $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Delete CashBook Trx',$cbtrx->DocNo,'Failed');
    ...
    public function approve(){
        ...
        foreach ($ids as $id) {
            ...
            if($cbtrx->TrxStatus < 2){
                ...
                if ($rs) {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Approve CashBook Trx',$cbtrx->DocNo,'Success');
                    $infos[] = sprintf("Data Transaksi: %s (%s) sudah di-Approved", $cbtrx->DocNo, $cbtrx->TrxDescs);
                } else {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Approve CashBook Trx',$cbtrx->DocNo,'Failed');
    ...
    public function unapprove(){
        ...
        foreach ($ids as $id) {
            ...
            if($cbtrx->TrxStatus == 2 && $cbtrx->CreateMode == 0){
                ...
                if ($rs) {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Un-Approve CashBook Trx',$cbtrx->DocNo,'Success');
                    $infos[] = sprintf("Data Transaksi: %s (%s) sudah di-Unapproved", $cbtrx->DocNo, $cbtrx->TrxDescs);
                } else {
                    $log = $log->UserActivityWriter($this->userCabangId,'cashbook.cbtrx','Un-Approve CashBook Trx',$cbtrx->DocNo,'Failed');
    ...
    public function approve1($id = null){
        ...
        redirect_url("cashbook.cbtrx/view/".$id);
    }
    ...
    public function cetakpdf($id = null){
        ...
        $loader = new TrxType($cbtrx->TrxTypeCode);
