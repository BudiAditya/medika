...
if ($invoice->Id > 0){
    $title = 'UPDATE NOTA PENJUALAN';
}else{
    $title = 'ENTRY NOTA PENJUALAN';
}
?>
<br />
<div id="p" class="easyui-panel" title="<b><?php print($title);?></b>" style="width:100%;height:100%;padding:10px;" data-options="footer:'#ft'">
    <table>
        <tr>
            <td style="width: 100%">
                ...
                <table cellpadding="0" cellspacing="0" class="tablePadding" align="left" style="font-size: 13px;font-family: tahoma">
                    <tr>
                        <td>Tanggal</td>
                        <td><input class="easyui-datebox bold" id="Tanggal" name="Tanggal" value="<?php print($invoice->Tanggal);?>" data-options="formatter:myformatter,parser:myparser" style="width:100px;">
                            &nbsp;&nbsp;<b>
                            <?php
                    ...
                    <tr>
                        <td>Customer</td>
                        <td><input class="easyui-combogrid" id="RelasiId" name="RelasiId" style="width: 250px" value="<?php print($invoice->RelasiId);?>" required autofocus/>
                            ...
                            <input type="hidden" id="TrxStatus" name="TrxStatus" value="<?php print($invoice->TrxStatus == null ? 0 : $invoice->TrxStatus );?>"/>
                            <input type="hidden" id="TotalHpp" name="TotalHpp" value="<?php print($invoice->TotalHpp);?>"/>
                        </td>
                        <td>Jenis Pasien</td>
            ...
            <td colspan="2">
                <table cellpadding="0" cellspacing="0" class="tablePadding tableBorder" align="left" style="font-size: 12px;font-family: tahoma">
                    ...
                    foreach($invoice->Details as $idx => $detail) {
                        ...
                        $dtx = addslashes($detail->Id.'|'.$detail->KodeBarang.'|'.str_replace('"',' in',$detail->NamaBarang).'|'.$detail->QtyKeluar.'|'.$detail->Satuan.'|'.$detail->Harga.'|'.$detail->DiskonPersen.'|'.$detail->DiskonNilai.'|'.$detail->IsFree.'|'.$detail->SubTotal.'|'.$detail->HppNilai);
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="right bold" style="width: 30px" id="PajakPersen" name="PajakPersen" value="<?php print($invoice->PajakPersen != null ? $invoice->PajakPersen : 0); ?>"/>
                            <input type="text" class="right bold" style="width: 110px" id="PajakNilai" name="PajakNilai" value="<?php print($invoice->PajakNilai != null ? number_format($invoice->PajakNilai,0) : 0); ?>" readonly/></td>
...
<div id="dlg" class="easyui-dialog" style="width:1150px;height:200px;padding:5px 5px" closed="true" buttons="#dlg-buttons">
    <form id="fdt" method="post" novalidate>
        <table cellpadding="0" cellspacing="0" class="tablePadding tableBorder" style="font-size: 13px;font-family: tahoma">
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="bold" id="aSatuan" name="aSatuan" size="5" value="" readonly/>
                </td>
                <td>
...
<script type="text/javascript">
    ...
    $( function() {
        ...
        $("#DiskonPersen").change(function(e){
            hitMaster();
        });

        $("#PajakPersen").change(function(e){
            hitMaster();
        });
    ...
    function feditdetail(dta){
        ...
        $('#aQty').val(dtx[3]);
        $('#aQtyStock').val(0);
        $('#aDiskonPersen').val(dtx[6]);
        $('#aDiskonNilai').val(dtx[7]);
        $('#aSubTotal').val(dtx[9]);
        $('#aItemHpp').val(dtx[10]);
        $('#aIsFree').val(dtx[8]);
        if (Number(dtx[8]) == 0){
        ...
        $('#dlg').dialog('open').dialog('setTitle','Edit Detail');
    }
    ...
    function fdeldetail(dta){
        ...
        var kode = dtx[1];
        var barang = dtx[2];
        var urx = '<?php print($helper->site_url("apotek.invoice/delete_detail/"));?>'+id;
    ...
    function saveDetail(){
        ...
        var atxm = Number($('#aMode').val());
        var aid = Number($('#aId').val());
        var aitc = Number($('#aItemCode').val());
        var aitn = $('#aItemName').val();
        ...
        var oke = true;
        if ((aitc != '' && aqty > 0 && astt > 0) || (aitc != '' && aqty > 0 && aisf == 1)){
            //if (confirm('Apakah data input sudah benar?')) {
                ...
                $.post(url, {
                    Tanggal: $("#Tanggal").textbox("getValue"),
                    TrxNo: $("#TrxNo").val(),
                    RelasiId: custId,
                    NoRm: $("#NoRm").textbox("getValue"),
                    JenisPasien: $("#JenisPasien").combobox("getValue"),
                    CaraBayar: $("#CaraBayar").combobox("getValue"),
                    KdDokter: $("#KdDokter").combogrid("getValue"),
                    JnsBeli: $("#JnsBeli").combobox("getValue"),
                    NoResep: $("#NoResep").textbox("getValue"),
                    NmPasien: $("#NmPasien").textbox("getValue"),
                    UmrPasien: $("#UmrPasien").textbox("getValue"),
                    NoHp: $("#NoHp").textbox("getValue"),
                    Uraian: $("#Uraian").textbox("getValue"),
                    SubTotal: $("#SubTotal").val().replace(/,/g,""),
                    DiskonPersen: $("#DiskonPersen").val().replace(/,/g,""),
                    DiskonNilai: $("#DiskonNilai").val().replace(/,/g,""),
                    PajakPersen: $("#PajakPersen").val().replace(/,/g,""),
                    PajakNilai: $("#PajakNilai").val().replace(/,/g,""),
                    BiayaNilai: $("#BiayaNilai").val().replace(/,/g,""),
                    ...
                    if (rst[0] == 'OK') {
                        //validasi detail
                        var aivi = rst[2];
                        if ((aitc != '' && aqty > 0 && astt > 0)||(aitc != '' && aqty > 0 && aisf > 0)) {
                            //proses simpan detail
                            if (atxm == 0){
                                var urz = "<?php print($helper->site_url("apotek.invoice/add_detail/")); ?>" + aivi;
                            }else{
                                var urz = "<?php print($helper->site_url("apotek.invoice/edit_detail/")); ?>" + aivi;
                            }
                            $.post(urz, {
                                aId: aid,
                                aItemCode: aitc,
                                aItemName: aitn,
                                aQty: aqty,
                                aHarga: Number($('#aHarga').val().replace(/,/g,"")),
                                aDiskonPersen: $('#aDiskonPersen').val(),
                                aDiskonNilai: $('#aDiskonNilai').val(),
                                aSatuan: $('#aSatuan').val(),
                                aSubTotal: astt,
                                aItemHpp: ahpp,
                                aIsFree: aisf
                                ...
                                var rsx = data.split('|');
                                if (rsx[0] == 'OK') {
                                    location.href = "<?php print($helper->site_url("apotek.invoice/add/")); ?>" + aivi;
                                } else {
                                    alert(data);
                                }
                            });
    ...
    function saveMaster () {
        ...
        invoiceId = "<?php print($invoice->Id == null ? 0 : $invoice->Id);?>";
        if (invoiceId == 0){
            var psn = 'Simpan Data Invoice?';
        }else{
            var psn = 'Update Data Invoice?';
        }
        if (custId > 0){
            if (confirm(psn)) {
                var url = "<?php print($helper->site_url("apotek.invoice/proses_master/")); ?>"+invoiceId;
    ...
    function printStruk(invId){
        var urx = "<?php print($helper->site_url("apotek.invoice/printStruk/")); ?>"+invId;
